...
class PhoneNumberService extends ApiService {
  ...
  async getAccountPhoneNumbers(): Promise<ListPhoneNumbersResponse> {
    ...
    return cachedResult.success ? cachedResult : this.getAccountPhoneNumbersFn();
  }
  ...
  async validatePhoneNumber(phoneNumber: string): Promise<ValidatePhoneNumberResponse> {
    ...
    // if we have a valid storage value use it, otherwise get from backend.
    return cachedResult.success ? cachedResult : this.validatePhoneNumberFn(phoneNumber);
  }

  getAccountPhoneNumbersFn = async (): Promise<any> => {
  ...
  validatePhoneNumberFn = async (phoneNumber: string): Promise<ValidatePhoneNumberResponse> => {
    const encodedParams: EncodedParams = {
      Token: encodeURIComponent(this.manager.user.token),
